...
Connector.getTrack = () => {
	...
	if (links.length >= 2) {
		...
		// Remove artist name if it's at the start
...
// Check if pause icon is visible (which means music is playing)
Connector.isPlaying = () => {
	// Look for the pause icon SVG path - it has the characteristic two rectangles
